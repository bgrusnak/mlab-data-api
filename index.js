...
MLab.prototype.updateDocument = function (options) {
  ...
  if (typeof options.id !== 'string' || options.id === null) throw new Error('document id is required');
  if (options.data === null) throw new Error('document data is required');
  return this.axios.put(this.makePath(['collections', options.collection, options.id], { database: options.database }), options.updateObject, {
    params: { apiKey: this._key }
  })
};
